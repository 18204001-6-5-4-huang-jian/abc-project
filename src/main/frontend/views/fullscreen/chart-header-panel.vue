...
<!-- 			<div class="chart-header-chart-menu" @click="showChartList()">
			...
			<!-- <div class="chart-header-table-tab" v-if="!isMobile && !isTableChart" :class="{'active':showtype=='table'}" @click="setFullScreenWatchTable(true)">
			 ...
			 <div class="chart-header-caption-tab" v-if="!isMobile" :class="{'active':showtype=='caption'}" @click="toCaption()">
...
<script type="text/javascript">
	export default {
		...
		computed:{
			...
			isTableChart(){
				if(this.curChart){
					return this.curChart.chartType === 'table'
			...
			forceShowChartTab() {
				// 强制显示图表Tab
				if(this.curChart){
					return this.curChart.type === 'research_data_list'
						|| this.curChart.type === 'research_stock_card';
				}
				return false;
			},
		...
		methods:{
			...
			setFullScreenWatchTable(isWatch){
				if(isWatch){
					...
					GaHelper.sendEvent(GaHelper.FullScreen.viewData, this.dashboardName + "看板-" + this.curWatchChart_copy.title + "-data");
				}else{
					this.$store.commit('zb_changeShowType',"chart");
					GaHelper.sendEvent(GaHelper.FullScreen.viewChart, this.dashboardName + "看板-" + this.curWatchChart_copy.title + "chart");
				}
			...
			changeFullScreenWatchTable(){
				this.$store.commit('setFullScreenWatchTable',{isWatch:!this.isFullScreenWatchTable})
			},
			toCaption(){
			...
			destroyChartOrTable(){
				...
				if ($('#cc-chart-container').highcharts()) {
					$('#cc-chart-container').highcharts().destroy();
				}
				if ($("#cc-table-container").handsontable('getInstance')) {
					$("#cc-table-container").handsontable('getInstance').destroy();
				}
			...
			fileExport(type) {
				GaHelper.sendEvent(GaHelper.FullScreen.export, this.dashboardName + "看板-" + this.curWatchChart_copy.title + "-export");
				...
				if(this.curChart){
					if(type == 'excel'){
						...
						FileExport.export({
	                        type: 'png',
	                        data: {
	                            sId: this.isTableChart ? 'cc-table-container' : 'cc-chart-container',
